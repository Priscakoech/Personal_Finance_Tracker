...
class DataHandler:
    ...
    def generate_insight(self, category, data, user_name):
        ...
        if current_time - self.last_api_call_time < 15:
            raise Exception("Too many requests. Please wait before requesting again.")
        ...
        try:
            ...
            return MarkdownFormatter().format_text(text=response.choices[0].message.content)

        except Exception as e:
            print(f"Error generating insight: {e}")
            raise Exception("Could not generate insights. Try again later.")
